...
function isValidRoomCode(code) {
  return code && typeof code === 'string' && /^[A-Z0-9]{6}$/.test(code);
}
...
// ─── Host tokens for authentication ──────────────────────────────────────────
const hostTokens = new Map(); // roomCode -> token

function createRoom(hostId, hostName, hostAvatar = '🐼') {
  const code = generateRoomCode();
  const token = crypto.randomBytes(16).toString('hex');
  hostTokens.set(code, token);
  rooms[code] = {
    code,
    host: hostId,
    players: {},
...
// ─── Socket.io events with validation & auth ─────────────────────────────────
io.on('connection', (socket) => {
  ...
  socket.on('create-room', ({ name, avatar }) => {
    ...
    const safeAvatar = (avatar && typeof avatar === 'string' && avatar.length <= 2) ? avatar : '🐼';
    const { code, token } = createRoom(socket.id, safeName, safeAvatar);
    const room = rooms[code];
    room.players[socket.id] = { id: socket.id, name: safeName, score: 0, isDrawing: false, avatar: safeAvatar };
    socket.join(code);
  ...
  socket.on('join-room', ({ code, name, avatar }) => {
    ...
    if (Object.keys(room.players).length >= 10) {
      socket.emit('error', { message: 'Room is full (max 10)!' });
      return;
    }
    ...
    socket.emit('room-joined', { code: upperCode, playerId: socket.id, room: getRoomPublic(room) });
    ...
    socket.to(upperCode).emit('player-joined', { player: { id: socket.id, name: safeName, score: 0, avatar: safeAvatar }, room: getRoomPublic(room) });
    console.log(`[Room] ${safeName} joined room ${upperCode} (state: ${room.state})`);
  });

  socket.on('start-game', ({ token } = {}) => {
  ...
  socket.on('guess', ({ message }) => {
    ...
    const isCorrect = safeMessage.toLowerCase() === room.currentWord.toLowerCase();
    if (isCorrect) {
      ...
      const close = levenshtein(safeMessage.toLowerCase(), room.currentWord.toLowerCase()) <= 2;
      io.to(code).emit('chat', { name: player.name, message: safeMessage, correct: false, isClose: close, isSystem: false });
    }
  });
